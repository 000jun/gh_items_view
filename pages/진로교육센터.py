...
st.set_page_config(page_title="진로 내역", layout="wide")
st.header("진로 물품조사 페이지", divider='rainbow')

# 데이터프레임 처리
df = pd.read_excel('진로센터.xlsx') # 파일명 확인 : 행복센터, 무계센터 -> happy
...
merged_df['순'] = merged_df['순'].astype(int)
merged_df.sort_values(by='순', inplace=True)
...
with st.sidebar.header('저장 버튼'):
    if st.button('저장', help='검수수량 입력 후 저장 버튼을 누르면 데이터 저장'):
        result.to_excel('진로센터.xlsx', index=False)
        st.success('저장 완료')
